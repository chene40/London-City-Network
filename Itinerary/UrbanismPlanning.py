...
class UrbanismPlanning:
	def __init__(self, graph: Graph, stations: list):
		...
		self.islands = {}		# {zone#: [[island1], [island2]], ... } island is of type set containing the id of the island
	...
	def create_zones(self):
		...
		""" check parser file and fix this odd-looking entry ://// - how do you parse this???? """
	...
	def num_zones(self): 
		# print(f"The number of different zones there are is: {len(self.zones.keys())}.")
	...
	def islandsDFS(self, temp: list, node: int, visited: list, nodeZone: float):
		visited[node] = True
		temp.append(node)
		
		for node in self.graph.adjacent_nodes(node):
			for key, value in self.zones.items():
				if node in value: currNodeZone = key
					
			if currNodeZone != nodeZone: continue

			if not visited[node]:
				temp = self.islandsDFS(temp, node, visited, nodeZone)
		return temp
	...
	def create_islands(self):
		...
		for node in self.graph.get_nodes_name():
			for key, value in self.zones.items():
				if node in value: nodeZone = key

			if not visited[node]:
				temp = []
				islands.append({nodeZone: self.islandsDFS(temp, node, visited, nodeZone)})

		for island in islands:
			for key in island.keys(): zone = key

			if not zone in self.islands.keys():
				for value in island.values(): newValue = value
				self.islands.update({zone: [newValue]})

			else:
				cc = self.islands.get(zone)
				for value in island.values(): cc.append(value)

	def get_islands(self): return self.islands

	def get_islands_in_zone(self, zone: int): return self.islands.get(zone)

	def num_islands_in_zone(self, zone: int): return len(self.islands.get(zone))

	def zone_of(self, station):
		for key, value in self.zones.items():
			if station in value: return key
	...
	def connected_zones(self, zone: int):
		...
		for z in zones:
			...
			for node in adjS:
				...
				if not z0 in cZones and z0 != zone: cZones.append(z0)
		return cZones
